...
func set_web_duration(duration):
	webbed_duration = duration
...
func _on_Area_body_entered(body):
	if !body.is_in_group("Player"):
		return
	queue_free()
	body.health = clamp(body.health-(damage), 0, Global.max_health)
	body.appy_webbed(webbed_duration)
